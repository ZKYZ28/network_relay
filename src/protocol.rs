...
impl Protocol {
    ...
    const NOM_UTILISATEUR: &'static str =
        "(?P<nom_utilisateur>(?P<lettre_chiffre>[A-Za-z0-9]){5,20})";
    ...
    const ID_DOMAINE: &'static str =
        "(?P<id_domaine>[0-9]{1,5}@(?P<domaine1>(?P<lettre_chiffre6>[A-Za-z0-9]|[.]){5,200}))";

    pub fn from_message(message: &str) -> Option<String> {
        let echo_regex = Regex::new(&format!(
            "^ECHO{}{}{}{}{}",
            Protocol::ESP,
            Protocol::PORT,
            Protocol::ESP,
        ...
        .ok()?;
        let send_regex = Regex::new(&format!(
            "^SEND{}{}{}{}{}(({})|({})){}{}{}",
            Protocol::ESP,
            Protocol::ID_DOMAINE,
            Protocol::ESP,
    ...
    pub fn decomposer(string: &str, typ: &str) -> Result<Vec<String>, &'static str> {
        let regex_str = match typ {
            ...
            "send" => format!(
                ...
                Protocol::NOM_DOMAINE,
                Protocol::ESP,
                "([A-Za-z0-9]{5,20})@([A-Za-z0-9.]{5,200})",
                Protocol::TAG_DOMAINE,
                Protocol::ESP,
                Protocol::MESSAGE_INTERNE,
                Protocol::CRLF
    ...
    //TODO
    pub fn get_send_map(msg : &str) -> Option<String> {
        let regex_str = format!(
            "^SEND{}{}{}{}{}(({})|({})){}(?P<destinationDomain>[A-Za-z0-9.{{5,200}}]){}{}",
            Protocol::ESP,
            Protocol::ID_DOMAINE,
            Protocol::ESP,
            Protocol::NOM_DOMAINE,
        ...
        let destination_domain = captures.name("destinationDomain").unwrap().as_str().to_owned();
